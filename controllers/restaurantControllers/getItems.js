...
export const getItemsController = async (req, res) => {
  try {
    const restaurant = await restaurantModel.findById(req.params.id).populate({
      path: "menu",
      select:
        "_id name price description totalSold dailySales lastSoldDate popularityScore discount discountStart discountEnd profitMargin stockPercentage lowStockAlert status category image costPrice stock maxStock minStock preparationTime unit weight createdAt updatedAt expiryDate supplier tags",
    });
